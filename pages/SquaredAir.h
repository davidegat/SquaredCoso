...
/*
   ...
   - Effetto foglie animato solo parte bassa pagina (DMA-friendly)
*/
...
// ---------------------------------------------------------------------------
// EXTERN (nel core di SquaredCoso)
// ---------------------------------------------------------------------------
...
static String airVerdict(int& categoryOut) {
  ...
  categoryOut = worst;
  if (worst < 0) {
    return (g_lang == "it") ? String("Aria: dati non disponibili") : String("Air: data unavailable");
  }
  char bufCat[20];
  char bufTip[40];
  if (g_lang == "it") {
...
static bool fetchAir() {
  ...
  String url = String("https://air-quality-api.open-meteo.com/v1/air-quality?latitude=")
               + g_lat + "&longitude=" + g_lon + "&hourly=pm2_5,pm10,ozone,nitrogen_dioxide&timezone=auto";
  String body;
...
// ---------------------------------------------------------------------------
// PARTICLE SYSTEM – FOGLIE ORIZZONTALI
// ---------------------------------------------------------------------------
#define N_LEAVES 100
...
static void tickLeaves(uint16_t bg) {
  ...
  for (int i=0; i<N_LEAVES; i++) {
    Leaf& L = leaves[i];
    if (L.oldY >= 300) drawLeafFX(L.oldX, L.oldY, L.size, bg);
    L.x += L.vx;
    L.phase += 0.5f;
...
static void pageAir() {
  bool it = (g_lang=="it");
  drawHeader( it ? String("Aria a ") + sanitizeText(g_city)
